...
from agentx.subsystems.sentinel_x.governance import identity, registry
...
def enforce(agent_id: str, action: str) -> Callable[[F], F]:
    ...
    def decorator(fn: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ...
            # 1. Map SENTINEL actions to Agent X risk levels
            risk = "low"
            reversible = True
            if action in ("create_branch", "trigger_patch"):
                risk = "medium"
                reversible = False # Making a PR/commit is an external write
            
            # 2. Call the main Agent X Governor
            # For this integration, we assume a default case_level of 3 (Act on reversible things)
            # but require level 4 for irreversible patches. 
            from agentx import governor
            verdict = governor.assess(
                action=action, 
                capability=None, 
                case_level=4 if risk == "low" else 3, # Mock case level for demo
                risk=risk,
                confidence=0.99
            )
            ...
            if not verdict.allow:
                _log({"ts": ts, "agent": agent_id, "action": action, "decision": "blocked",
                      "reason": verdict.rule, "explain": verdict.explain})
                raise GatewayDenied(f"Agent X Governor denied: {verdict.explain}")
